public class Player{
    ...
    public Player(State home){
	_homeState = home;
	_party = home.getMajority();
	_income = home.getAvgInc() * 10;
    }
    ...
    public int legisAcheivement(Legislation bill){
	...
	if (bill.getForeign()==0){//intervention
	    if(_homeState.getHawks()>0.5){_acheivements+=1;}
	    if(_homeState.getBear()>0.5){_acheivements-=1;}
 	}
	...
	if (bill.getEcon()==0){//populist
	    if(_homeState.getExLeft()>0.5){_acheivements+=1;}
	...
	if (bill.getSoc()==0){//no regulation, libertarian
	    if(_homeState.getExLeft()>0.4){_acheivements-=1;}
	    if(_homeState.getExRight()>0.7){_achievements+=1;}
	}
	else if (bill.getSoc()==1){//left-leaning
	    if(_homeState.getCompLeft()>0.5){_acheivements+=1;}
	    ...
	    if(_homeState.getExLeft()>0.7){_acheivements+=1;}
